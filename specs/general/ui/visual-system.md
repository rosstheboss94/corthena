...
**Related:** [UI foundation](README.md), [Workspaces](workspaces.md), [Visualization](visualization.md), [Visualization quality](../quality/visualization.md), [Migration baseline](../../history/migration/migration-baseline.md)
...
API Tokens, file mapping, Massive pulls, schedules, progress, validation, and
recovery use the same modal/control/state primitives as the rest of the shell.
Multi-step flows show a compact step label, current dataset/provider context,
bounded scrollable content, and persistent Back/Cancel/Confirm actions. Progress
includes determinate values when known and textual phase/status when not; a
cancelling state remains distinct from cancelled and complete.

Credential screens render only a fixed saved/not-saved status and safe test
metadata. Never render the stored token, its prefix/suffix, or a mask whose
length reflects the token. Transient entry uses a dedicated secret buffer
outside published `AppState`, clears on close/cancel/submission/terminal
failure, and is excluded from canonical state fixtures and captures. Place the
plaintext-storage warning adjacent to the save/replace confirmation and pair
warning color with icon and explicit text.

File mapping aligns source columns, detected roles, user-selected roles, types,
and representative validation state in a dense table. Pull and schedule forms
keep symbol, interval, session, adjustment, range, and cadence policies visible
at confirmation. Authentication, entitlement, rate-limit, validation, failure,
reconnect, and recovery states use distinct headings/actions and never reveal
credentials, Authorization values, raw provider bodies, or secret-bearing URLs.
...
Real Data ingestion adds canonical scenarios for API-token settings, file
mapping, Massive pull configuration, schedule editing, active progress,
validation error, authentication error, and recovered/reconciled state. Each is
captured at 1280x720 and 1920x1080 at 100%, 150%, and 200% scale with a token-free
fixture. Their manifest and PNG family become required evidence together; the
existing channel tolerance and differing-pixel ratio remain unchanged unless
an explicitly approved manifest establishes stricter values.
