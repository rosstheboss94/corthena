...
**Related:** [Repository guidance](../../AGENTS.md), [Specification index](../README.md), [Contract examples](../examples/agent-facing-contracts.md), [Design pattern](design-pattern.md), [API](api.md), [Quality](quality/README.md)

This document is the mandatory baseline for every Python/Cython task. Read it
before planning, editing, reviewing, testing, or selecting
...
repository root. Read the owning route specifications separately as required by
`AGENTS.md`.
...
- The request/result models referenced by the methods being called; for Phase
  6--9, use the owning models under `research`, `data_experiments`,
  `jobs_results`, or `models_inference`

Do not load the simulator, effects runtime, reducer, lifecycle, or renderer
unless the task changes their behavior.

### UIClient adapter implementation

Read:

- `src/corthena/ui/client/protocol.py`
- The owning Phase 6--9 request/result models
...
### UI workflow internals

Read:

- The owning implementation file under `src/corthena/ui/research/`,
  `data_experiments/`, `jobs_results/`, or `models_inference/`
...
Read `src/corthena/ui/native/protocol.py` and `native/models.py` when calling
the interchangeable native capability. Read `native/raylib.pyi` when directly
constructing or configuring the concrete Raylib adapter. Native/library values
remain implementation-local; package-level compatibility aliases do not replace
these canonical agent-facing interfaces.
