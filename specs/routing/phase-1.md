...
- Required: `AGENTS.md`, Phase 1 in `roadmap.md`, `design-pattern.md`,
  ...
  `ui/foundation.md`, and `ui/foundation-shell-state.md`.
- Conditional: read `api.md` only if a public or process boundary is
  ...
  the empty scaffold.
- Implement with `$build-corthena-raylib-ui`, then apply
...
- Scope: add the named workstation project entry point and the smallest owned
  `corthena.ui` package needed for an empty Raylib/Raygui frame loop;
...
- Completion evidence: the named entry point and UI package exist; source
  ...
  checks formed the accepted evidence. Phase 1 is complete.
