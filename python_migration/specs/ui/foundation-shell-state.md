...
failure. Its visual acceptance target is the manifest-owned legacy Go
...
Phase 3 ports the complete legacy Go application-shell renderer so its shared
...
This deliberate migration deviation brings only behavior owned directly by
`internal/frontend/nativeui/shell_windows.go` forward. Phase 4 continues to own
