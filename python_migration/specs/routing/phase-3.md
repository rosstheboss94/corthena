...
The complete legacy Go shell surface is deliberately ported, including typed
...
- The Python lossless capture passes the first-party RGBA comparison against
  the manifest-owned Go `phase3_application_shell.png` using its recorded seed,
...
- Every applicable configured common, concurrency, visualization, Windows
  ...
  build/import gate passes, and final review has no unresolved findings. Keep
  Phase 3 Pending until the implementation and all evidence exist.
