...
declarations and tool configuration. `uv.lock` owns resolved versions once the
...
The compatibility spike chooses exact versions and records them only in lock
and project files.

## Windows Compatibility Spike Acceptance

Before `pyproject.toml` or `uv.lock` select runtime or native versions, run a
Windows compatibility spike for the initial shell. Its structured report must
select and record the CPython version, Windows architecture, Python
Raylib/Raygui binding pair, compiler/toolchain, native dependency behavior,
and the exact locked versions only after the spike passes.
...
An unsuccessful spike does not select a binding pair or lock exact versions;
record the failure evidence and continue evaluation.
