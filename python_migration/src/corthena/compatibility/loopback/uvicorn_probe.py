"""Uvicorn-backed loopback compatibility probe implementation."""
...
async def _handshake(port: int, runtime: RuntimeCapabilities) -> LoopbackEvidence:
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(3.0)) as client:
        ...
        if response.json() != {"api_version": "v1", **asdict(runtime)}:
...
class UvicornLoopbackProbe:
    """Own the bounded Uvicorn server thread used by Phase 0."""
    ...
    def run(self, runtime: RuntimeCapabilities) -> LoopbackEvidence:
        ...
        config = uvicorn.Config(_create_app(runtime), log_level="warning", lifespan="off")
