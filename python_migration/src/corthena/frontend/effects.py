...
from corthena.frontend.client import FrontendClient, RequestCancelledError
...
class EffectsRuntime:
    ...
    def __init__(self, client: FrontendClient, config: RuntimeConfig | None = None) -> None:
        if config is None:
            config = RuntimeConfig()
        self._client = client
        self._config = config
        self._effects: queue.Queue[LoadSnapshot | None] = queue.Queue(config.effect_capacity)
    ...
    def enqueue(self, effect: UIEffect) -> EnqueueResult:
        ...
        with self._lock:
            ...
            if isinstance(effect, CancelRequest):
            ...
            if effect.request_id in self._cancellations:
                return EnqueueResult(
                    EnqueueState.BUSY,
                    RuntimeBusy(effect.request_id, effect.generation),
                )
