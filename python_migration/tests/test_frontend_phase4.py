...
from corthena.frontend.serialization import decode_action, encode_action
from corthena.frontend.shell import project_dock_drop_targets, project_shell
...
def test_live_state_uses_revision_checked_workspace_layouts() -> None:
    ...
    stale, _ = reduce(state, ActivateDockPanel("data-1", initial.revision + 1))
    assert stale is state
    state, _ = reduce(state, ActivateDockPanel("data-1", initial.revision))
    assert workspace_layout(state).revision == 1
    state, _ = reduce(state, CloseDockPanel("data-4", 1))
    assert workspace_layout(state).hidden[0].id == "data-4"
    state, _ = reduce(state, ReopenDockPanel("data-4", "data-stack-root", 2))
    state, _ = reduce(state, SetDockMaximized("data-1", 3))
    view = project_shell(state)
    assert len(view.dock_stacks) == 1
    assert view.maximized_panel_id == "data-1"
